...
一時的に割り当てられたURLが、ローカルのポート80番につながっています。
...
ngrokを使用する際は、以下の点にご注意ください。

### 無料プランの制限

- URLは起動ごとにランダムに生成されます
- セッションは一定時間で切断されます
- 同時接続数に制限があります

### 公開範囲

生成されたURLを知っている人は誰でもアクセスできます。またランダムURLであるものの、内容は意味のある文字列で構成されているため、辞書攻撃で特定される懸念もあります。そのためプロジェクトによっては以下のような制限も必要です。
...
無料プランでも十分に実用的ですが、頻繁に使用する場合は有料プランも検討する価値があります。固定URLの使用や、より高度なセキュリティ設定が可能になります。

開発効率を上げるためにも、ぜひngrokを活用してみてください。
