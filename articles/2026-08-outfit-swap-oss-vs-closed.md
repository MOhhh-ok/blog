---
title: "動画の着せ替えは全段OSSでいけるか (Qwen-Image-Edit + Wan2.2-Animate)"
emoji: "👗"
type: "tech"
topics: ["ai", "generativeai", "opensource", "wan", "qwen"]
published: true
---
...
![動画着せ替えパイプライン全体像](https://raw.githubusercontent.com/mohhh-ok/blog/main/src/content/posts/2026/07-31-outfit-swap-pipeline.svg)
...
![タータンブレザーの参照画像](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/01-ref-tartan-blazer.webp)

![着物の参照画像](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/02-ref-kimono.webp)

![種になる人物静止画](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/03-seed-portrait.webp)

![モーション源となる元動画](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/10-source-video.webp)
...
![Kolors の着物は破綻する](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/06-s2-kolors-kimono.webp)
...
![Seedream は着物を正しく着せる](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/07-s3-seedream-kimono.webp)

instruction 編集型はこの服型の壁を越えられます。クローズドの Seedream は足首丈・広袖・衿合わせ・帯まで一発で正解しました。

![Qwen-Image-Edit のブレザーは Kolors と同水準](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/08-s4-qwen-tartan.webp)

本命の Qwen-Image-Edit (Apache-2.0) は、ブレザーでは Kolors と同水準の再現度が出ました。全段 OSS の望みをつなぐ結果です。

![Qwen-Image-Edit の着物はひざ丈になった](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/09-s5-qwen-kimono.webp)

一方、着物は丈がひざ丈で止まり、裾から元のダークパンツが覗きました。

当初これを Qwen の失敗と解釈したのですが、参照画像を見直すと**そもそも生成した着物素材自体がひざ丈**でした。つまり Qwen は参照画像に忠実だっただけです。プロンプトでは ankle-length と指示していたので、ここで観測されたのは「参照とプロンプトが矛盾したとき、Qwen は参照を優先し、Seedream はプロンプトを優先した」という挙動差であって、着せ替え能力の優劣ではありません。正しい丈の参照での再試行は追試 1 で行い、そちらで決着します。
...
![左: replace、右: move。同じ着物静止画を両モードに通した比較](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/34-video-kimono-replace-vs-move.webp)
...
![Qwen で着せたボールガウンの種静止画](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/51-qwen-ballgown.webp)

種静止画は Qwen-Image-Edit 製。ガウンの裾が人物の輪郭から左右に大きく張り出しており、これを両モードに通します。

![左: replace はガウンが切り詰められ破綻、右: move は広がりを保持](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/52-video-ballgown-replace-vs-move.webp)
...
![左: 全段OSS、右: Kling O3 edit](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/20-video-tartan-oss-vs-kling.webp)
...
![左: Seedream起点は足首丈、右: Qwen起点は静止画のひざ丈がそのまま伝播](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/21-video-kimono-seedream-vs-qwen.webp)
...
![Kling O3 edit はプロンプトのみで着物に差し替える](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/22-video-kimono-kling.webp)
...
![作り直した足首丈の着物参照](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/30-ref-kimono-long.webp)

![Qwen-Image-Edit は足首丈を正しく出した](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/31-s5b-qwen-kimono-long.webp)

この参照で Qwen-Image-Edit を再実行すると、**裾が足首まで届き、パンツ露出のない正しい着物**が出ました。前回のひざ丈はやはり「参照への忠実さ」だったわけです。

留意点として、作り直した参照は flat lay ではなくゴーストマネキン風の吊り下げ提示になったため、改善の原因が「丈」なのか「提示方法」なのかは厳密には分離できていません。ただ実務上は「参照を正せば出る」ことが確認できれば十分です。

![全段OSSで着物への差し替えが成立した動画](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/33-video-kimono-long-oss.webp)
...
![街角背景の元人物](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/40-seed-portrait-bg.webp)

![Qwen で着せ替え。背景は意味的には保持されるがピクセルは別物](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/42-qwen-tartan-bg.webp)

まず静止画段。Qwen の着せ替えは、建物・車列・標識の配置という**意味レベルでは背景を保持**しますが、拡大すると歩行者の姿勢や車の細部が変わっており、**ピクセルレベルでは別物**です。instruction 編集型が画像全体を再生成する構造の帰結で、これが次の動画段の結果を分けます。
...
![左: replace mode、右: Kling。どちらも背景保持](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/44-video-bg-replace-vs-kling.webp)
...
![左: 元動画、右: move mode。背景が似て非なるものに置き換わる](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/45-video-bg-source-vs-move.webp)
...
![シード起因のソフトフォーカス例](https://raw.githubusercontent.com/mohhh-ok/blog-examples/main/2026/07-31-outfit-swap-oss-vs-closed/output/91-artifact-flux-softfocus.webp)
